    ...
    <div class="w-full max-w-sm p-5 bg-white rounded-lg border border-gray-300">
        ...
        <div v-if="title" class="flex justify-between items-center">
            ...
            <div v-if="tag">
                <p
                    :class="{
                        'bg-yellow-300': tag.color === 'yellow',
                        'bg-green-300': tag.color !== 'yellow',
                    }"
                    class="p-1 text-sm text-gray-900 rounded-lg"
                >
                    {{ tag.label }}
                </p>
            </div>
        ...
        <div v-if="content" class="mt-5">
            ...
            <div
                v-else-if="contentType === 'linkText'"
                v-for="item in content"
                :key="item"
            >
                <a
                    :href="item.link"
                    class="text-sm text-gray-600 p-1 bg-black/10 rounded-md"
                    >{{ item.text }}</a
                >
            </div>
        </div>
        ...
        <div
            ...
            class="mt-6 space-x-3 flex items-center justify-center"
        >
            <div
        ...
        <div v-else>
            <a
                ...
                :key="index"
                :href="btn.link"
                class="mt-6 w-full bg-black text-white px-4 py-3 rounded-lg flex items-center justify-between hover:bg-gray-800 transition-colors"
            >
                <span>{{ btn.label }}</span>
                <svg
